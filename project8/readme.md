...
### 1) `#define RATIO_THR 0.7`  
: ratio-based threshold used while running SIFT  

### 2) `#define THR 0.2`  
: threshold used when finding inliers  
...
### 9) `void findPairs(vector<KeyPoint>& keypoints1, Mat& descriptors1, vector<KeyPoint>& keypoints2, Mat& descriptors2, bool crossCheck, bool ratio_threshold)`
...
### 2) `HoughLines(InputArray image, OutputArray lines, double rho, double theta, int threshold, double srn = 0, double stn = 0, double min_theta = 0, double max_theta = CV_PI )`
