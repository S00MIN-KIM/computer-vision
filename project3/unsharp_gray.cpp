...
int main() {
    ...
    output = unsharpMask(input_gray, 3, 3, 3, "mirroring", 0.5); //Boundary process: zero-paddle, mirroring, adjustkernel
...
Mat unsharpMask(const Mat input, int n, float sigmaT, float sigmaS, const char* opt, float k) {
    ...
    denom = 0.0;
    for (int a = -n; a <= n; a++) {  // Denominator in m(s,t) //a=i,b=j,s,t=n
        for (int b = -n; b <= n; b++) {
