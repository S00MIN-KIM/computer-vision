...
int main() {
	...
	for (int y = 0; y < input.rows; y++)
		for (int x = 0; x < input.cols; x++) {
			...
			samples.at<float>(y + x * input.rows, 3) = (float)(y) / input.rows / 2;
			samples.at<float>(y + x * input.rows, 4) = (float)(x) / input.cols / 2;
		}
	...
	for (int y = 0; y < input.rows; y++)
		for (int x = 0; x < input.cols; x++)
		{
			...
			for (int z = 0; z < 3; z++) 
				new_image.at<Vec3b>(y, x)[z] = centers.at<float>(cluster_idx, z) * 255;
		}
